...
(function (win, doc) {
    ...
    let calendar = new FullCalendar.Calendar(calendarEl,
      {
        ...
        eventClick: function (info) {
          if (perfil == 'manager') {
            // IF verificava se o pedido estava pendente e levava para tela de aprovar pedido
            //if(info.event.borderColor == 'blue')
            ///{
              //win.location.href = `aprovarPedido.php?id=${info.event.id}`;
              ...
              win.location.href = `statusPedidos.php?id=${info.event.id}&title=${info.event.title}&clienteEmail=${info.event.extendedProps.clienteEmail}&provedorEmail=${info.event.extendedProps.provedorEmail}&preco=${info.event.extendedProps.precoServico}&status=${info.event.borderColor}&start=${horaInicial}&end=${horaFinal}`;
            //}
          }
          else if(perfil == 'user') 
          {
            ...
            win.location.href = `statusPedidos.php?id=${info.event.id}&title=${info.event.title}&clienteEmail=${info.event.extendedProps.clienteEmail}&provedorEmail=${info.event.extendedProps.provedorEmail}&preco=${info.event.extendedProps.precoServico}&status=${info.event.borderColor}&start=${horaInicial}&end=${horaFinal}&rating=${info.event.extendedProps.rating}`;
          }
        },
      });
    calendar.render();
  ...
  if(doc.querySelector('main.cadastroContinuacaoPage')){
  ...
  //Pagina Status do Pedido
  if(doc.querySelector('main.logadoPage div.conteudo.StatusPedido'))
  {
    let btAvaliar = doc.querySelector('#avaliar')

      //pegando a url do sistema
      let pathArray = window.location.pathname.split('/');
      
      //verificando se é a página de usuário
      if(pathArray[3] == 'user')
      {
        let teste = findGetParameter('rating');
        if(teste ==1 || teste ==2 || teste ==3 || teste ==4 || teste ==5)
        {
          //alert('Serviço avaliado');
          btAvaliar.style.display = 'none';
        }
      }
  }
