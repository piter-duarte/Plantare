          ...
          if (perfil == 'manager') {
           // IF verificava se o pedido estava pendente e levava para tela de aprovar pedido
            ...
            ///{
              ...
              let horaInicial = info.event.start;
              //let teste2 = new Date(horaInicial).toLocaleString("pt-br", {timeZone: "America/Sao_Paulo"});
              let teste2 = convertDateToUTC(horaInicial);
              alert(teste2.getMonth());
              ...
              //2022-12-09 09:00:00
              win.location.href = `statusPedidos.php?id=${info.event.id}&title=${info.event.title}&clienteEmail=${info.event.extendedProps.clienteEmail}&provedorEmail=${info.event.extendedProps.provedorEmail}&preco=${info.event.extendedProps.precoServico}&status=${info.event.borderColor}&start=${info.event.start}`;
            //}
          }
          else if(perfil == 'user') {
            if (info.event.borderColor == 'green' && info.event.extendedProps.rating == null) {
              //alert('Avaliar ' + info.event.extendedProps.provider_key);
              win.location.href = `avaliarProfissional.php?id=${info.event.id}&provedorEmail=${info.event.extendedProps.provedorEmail}`;
            }
          }
        },
      });
